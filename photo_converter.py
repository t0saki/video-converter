from video_converter import in_format as video_extensions, setup_logging, process_directory
import logging
import traceback
import subprocess
import datetime
...
#                     default='convert_images.log', help='Log file')
...
def process_image(args):
    ...
    try:
        ...
        if resolution > max_resolution:
            ...
            # Otherwise, convert the image as is
            pass
...
def convert_images(source_dir, target_dir, crf, max_resolution, max_workers=1):
    ...
    #     image_files.extend(list(source_dir.glob('**/' + image_extension)))

    image_files = [f for f in source_dir.rglob('*') if f.suffix.lower() in image_extensions and '@' not in str(f)]

    # remove redundant .MOV live photos
    live_photos = [f for f in source_dir.rglob('*') if f.suffix.lower() == '.mov' and f.with_suffix('.HEIC') in image_files or f.with_suffix('.heic') in image_files]
    ...
    # Convert videos
    process_directory(source_dir, target_dir, delete_original=False, ffmpeg_args=args.video_ffmpeg_args)
    ...
    for live_photo in live_photos:
        ...
        target_live_photo = target_live_photo.with_suffix('.mkv')
